...
def load_routes():
    ...
    if not isinstance(
        ...
        raise RuntimeError(
            "❌ skill_routes.json顶层必须是JSON object："
            f"{ROUTES_FILE}"
        )
...
def load_skills():
    ...
    for path in skill_files:
        ...
        # Skill ID = 文件名
        #
        # skill_routes.json也是按照文件名引用Skill。
        #
        # 因此禁止同名文件静默覆盖。
        # ----------------------------------------------------
        ...
        if skill_name in catalog:
            ...
            raise RuntimeError(
                "❌ 检测到重复Skill文件名："
                f"{skill_name}\n"
                f"Path 1: {previous_path}\n"
                f"Path 2: {relative_path}"
            )
