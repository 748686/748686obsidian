...
def ocr_text_quality_check(data):
    ...
    由于 OCR 对纯图片有时会产生随机误识别，
    因此这里只把明确的中文 / 英文识别结果
    作为主要拦截条件。
    """

    if Image is None:
        return True, "Pillow 不可用，跳过 OCR"

    if pytesseract is None:
        return True, "pytesseract 不可用，跳过 OCR"
