...
必须：
    ...
    1. 所有 Router 选中的 Skill 都出现
    2. 不得遗漏 Skill
    3. 不得增加额外 Skill
    4. 不得修改 Skill 名称
    5. 不得重新选择 Skill
    6. 必须保持 Router 的最终 Skill 集合

如果 Analysis 验证失败：

    第 1 次失败
        ↓
    将具体错误反馈给 Analysis AI
        ↓
    第 2 次生成
        ↓
    再次验证
...
# Analysis 内容生成成功，
# 但 Python 验证发现 Skill / Section 等错误时的重试
...
# ======================================================================
# EXTRACT SELECTED SKILLS SECTION
# ======================================================================

def extract_selected_skills_section(
    text: str,
) -> str:
    """
    提取：
...
# ======================================================================
# EXTRACT SKILL NAMES FROM SECTION
# ======================================================================

def extract_skill_names_from_section(
    section_text: str,
    known_skills: list[str],
) -> list[str]:
    """
    从 Selected Skills section 中提取 Skill 名称。

    只识别当前 Router 实际选择的 Skill。
...
def validate_existing_analysis(
    ...
    selected_section = extract_selected_skills_section(
        text
    )

    if not selected_section.strip():
        return False
...
中明确、逐字列出本次 Router 实际选择的全部 Skills：
...
def validate_generated_analysis(
    ...
    #
    # 这里使用已知 Skill 名称集合，
    # 防止 AI 在 Selected Skills 中偷偷加入
    # 其他 Skill。
    # --------------------------------------------------------------

    # 当前 Task 4 的 Selected Skills section
    # 只需要保证 Router 的 Skills 全部存在。
    #
    # 对未知字符串不进行误判，
    # 因为正文可能包含普通文字。
    #
    # 因此额外 Skill 的严格检查放在：
    # known Skill catalog 可用时完成。
    #
    # 当前调用方会继续执行最终结构验证。


# ======================================================================
# VALIDATE GENERATED ANALYSIS WITH FULL SKILL CATALOG
# ======================================================================
...
def validate_generated_analysis_strict(
    ...
    1. Router Skills 全部存在
    2. Selected Skills 不得出现其他已知 Skill
    3. Skill 数量必须与 Router 完全一致
    """
    ...
    found_known_skills: list[str] = []

    for skill_name in skill_catalog.keys():

        if skill_name in selected_section:

            found_known_skills.append(
                skill_name
            )
    ...
    if extra_skills:
        ...
        raise RuntimeError(
            "Generated analysis Selected Skills "
            "section contains unselected Skill(s):\n"
            + "\n".join(
                f"   - {skill_name}"
    ...
    if len(found_set) != len(
        expected_set
    ):

        raise RuntimeError(
            "Generated analysis Selected Skills "
            "count does not match Router selection.\n"
            f"Expected : {len(expected_set)}\n"
...
def process_event(
    ...
    for validation_attempt in range(
        ...
        try:
            ...
            if (
                validation_attempt
                < MAX_ANALYSIS_VALIDATION_RETRIES
            ):

                print(
                    "   🔁 Requesting Analysis AI "
                    "to repair output..."
                )
    ...
    # --------------------------------------------------------------
    # 三次 Analysis Validation 全部失败
    # --------------------------------------------------------------

    raise RuntimeError(
        "❌ Event Analysis failed strict "
...
def final_validate_unit(
    ...
    for event_path in event_paths:
        ...
        # ----------------------------------------------------------
        # 如果提供 Skill Catalog，
        # 再检查 Selected Skills section
        # 至少不是空的。
        # ----------------------------------------------------------

        if skill_catalog is not None:

            analysis_text = target.read_text(
                encoding="utf-8"
            )

            selected_section = (
                extract_selected_skills_section(
                    analysis_text
                )
            )
