...
def validate_final_event_ids(date, events):
    ids = [str(e.get("event_id", "")).strip() for e in events]
    if any(not x for x in ids):
        raise RuntimeError(f"❌ {date} Final EventUnit存在空event_id")
    if len(ids) != len(set(ids)):
        raise RuntimeError(f"❌ {date} Final EventUnit存在重复event_id")
    bad = [x for x in ids if not re.fullmatch(r"EVT-\d{8}-\d{6}", x)]
    if bad:
        raise RuntimeError(f"❌ {date} Final EventUnit存在非法Global ID：{bad[:20]}")
...
def mark_event_units_complete(
    ...
    p = language_dir(date) / EVENT_UNITS_COMPLETE_FILE
...
def run_task_3(date,language):
    ...
    if inspection["complete"]:
        print(f"♻️ TASK 3: EventUnits already complete | {date}/{lang}")
        return True
    merged_path=merged_clusters_path(date,lang)
    if not merged_path.exists(): raise RuntimeError(f"❌ TASK 3找不到TASK 2结果：{merged_path}")
    data=read_json(merged_path,None); final=data.get("clusters") if isinstance(data,dict) else None
    if not isinstance(final,list) or not final: raise RuntimeError("❌ TASK 2结果无效")
    events=build_event_units(date,final,news)
    validate_event_index_coverage(date,events,len(news)); validate_final_event_ids(date,events)
    save_aggregation_index(date,events)
    complete_existing_event_units(date,events,len(news))
    print(f"✅ TASK 3 COMPLETE | {date}/{lang} | events={len(events)}")
    return True


def main():
    ap=argparse.ArgumentParser(description="748686 Knowledge Task 3 - EventUnit V6.5.3")
    ap.add_argument("--date",required=True); ap.add_argument("--language",choices=["EN","ZH"],required=True)
    args=ap.parse_args(); run_task_3(args.date,args.language); return 0
