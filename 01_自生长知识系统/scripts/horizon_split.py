...
"""
748686 自生长知识系统
Horizon Bilingual Digest Splitter V11
...
8. 保留 Atomic 不完整时自动补建 / 重建。

9. 生成后再次严格验证。

10. 日期使用 UTC，与知识系统 Workflow 保持一致。
"""
...
# ------------------------------------------------------------
# 上游 Horizon 数据缺失时的最大检查次数
#
# 只针对：
#
#   前天 / 昨天
#
# 今天虽然也检查 3 次，但 3 次失败后必须报错，
# 绝不能 SKIP。
# ------------------------------------------------------------

UPSTREAM_CHECK_RETRIES = 3
...
#   Horizon 前置 Workflow
#   文件写入延迟
#   git checkout / pull 延迟
#
# 如果第一时间没有文件，不立即判定为没有数据。
# ------------------------------------------------------------

UPSTREAM_CHECK_INTERVAL = 10
...
def is_today_date(date: str) -> bool:
...
# ============================================================
# 检查历史日期的 Horizon 上游数据
# ============================================================

def wait_for_horizon_inputs(
    raw_root: Path,
    date: str,
    required: bool,
):
    """
    检查某一天的 Horizon 中文 / 英文日报。
    ...
    for attempt in range(
        ...
        if zh_input and en_input:

            print()
            print(
                "✅ Horizon upstream data is available."
            )
        ...
        if attempt < UPSTREAM_CHECK_RETRIES:

            print()
            print(
                "⏳ Horizon upstream data is not ready."
            )

            print(
                f"等待 {UPSTREAM_CHECK_INTERVAL} 秒后再次检查..."
            )
    ...
    print()
    print(
        "⚠️ Horizon upstream data is unavailable "
        "after all retries."
    )

    print(
        f"Final ZH: "
        f"{last_zh if last_zh else 'MISSING'}"
    )

    print(
        f"Final EN: "
        f"{last_en if last_en else 'MISSING'}"
    )
    ...
    if required:
        ...
        raise RuntimeError(
            f"{date} 是今天，Horizon 中文/英文日报在 "
            f"{UPSTREAM_CHECK_RETRIES} 次检查后仍不完整。"
            f" 今天禁止 SKIP，必须失败并提醒上游。"
        )
...
def process_date(
    ...
    upstream = wait_for_horizon_inputs(
        raw_root=raw_root,
        date=date,
        required=required,
    )
    ...
    if not upstream["available"]:
        ...
        return {
            ...
            "status": "skipped",
            "reason": (
                "Horizon upstream data unavailable "
                "after 3 checks"
            ),
            "zh": 0,
            "en": 0,
    ...
    zh_input = upstream["zh_input"]
    en_input = upstream["en_input"]
    ...
    print()
    print(
        f"Raw directory exists: {raw_dir}"
    )

    print(
        f"ZH Horizon: {zh_input}"
    )

    print(
        f"EN Horizon: {en_input}"
    )
    ...
    if atomic_dir.exists():
        ...
        print(
            "🆕 Atomic directory does not exist."
        )
...
def main():
    ...
    for date in dates:
        ...
        if date == today:
            ...
            print(
                f"  - {date}  ← TODAY / REQUIRED"
            )

        else:

            print(
                f"  - {date}  ← HISTORICAL / SKIP ALLOWED"
            )
    ...
    for date in dates:
        ...
        if required:
            ...
            print(
                "MODE: HISTORICAL / SKIP IF NO UPSTREAM"
            )
    ...
    print(
        f"Completed dates : {completed_count}"
    )

    print(
        f"Skipped dates  : {skipped_count}"
    )
    ...
    print()
    print(
        "✅ HORIZON BILINGUAL ATOMIC PROCESSING COMPLETE"
    )
