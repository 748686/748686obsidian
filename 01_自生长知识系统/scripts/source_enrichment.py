...
在 V7 基础上增加：
    ...
    1. 今天必须处理，不能因为 Atomic 暂时缺失而跳过
    2. 前天 / 昨天 Atomic 缺失时最多检查 3 次
    3. 前天 / 昨天 3 次检查仍无有效 Atomic
       → 视为当天没有上游数据
       → SKIPPED
       → 继续下一个日期
    4. Atomic 已经存在后，如果 Enriched 真正生成失败
       → 直接失败
       → 不把处理错误当成“没有数据”
    5. 每个日期成功后立即 Git commit + push
    6. SKIPPED 日期不执行 Git persistence
    7. 最终三日汇总允许历史日期 SKIPPED
    8. 今天绝对不能 SKIPPED
    9. Atomic 永远不主动创建 / 修改
   10. 日期统一使用 UTC
...
12. 原来的文章生成格式保持不变
13. 默认检查前天、昨天、今天
14. 前天 / 昨天没有 Atomic → 最多检查 3 次 → SKIPPED
15. 今天没有 Atomic → 最多检查 3 次 → Workflow 失败
16. Atomic 已存在 → 正常执行 Enrichment
17. Enrichment 真正失败 → Workflow 失败
18. 一个日期完整验证后立即 Git commit + push
19. 不等待其它日期
20. 历史日期 SKIPPED 不影响后续日期

============================================================
Git Push V8 修复
============================================================

如果 GitHub Actions checkpoint 时出现：

    ! [rejected] HEAD -> main (fetch first)

说明远端 main 已经领先当前 runner。
...
USER_AGENT = (
    "Mozilla/5.0 "
    "(compatible; 748686-Knowledge-Bot/8.0; "
...
ATOMIC_CHECK_RETRIES = int(
    os.environ.get(
        "ATOMIC_CHECK_RETRIES",
        "3",
    )
)

ATOMIC_CHECK_INTERVAL = int(
    os.environ.get(
        "ATOMIC_CHECK_INTERVAL",
        "10",
    )
)
...
def git_push_with_retry(
    ...
    *,
    ...
    解决 GitHub Actions 并发 checkpoint 导致：
    ...
    for attempt in range(
        ...
        try:
            ...
            raise RuntimeError(
                "Git push was rejected and "
                "automatic rebase failed. "
                "Repository may require "
...
# ============================================================
# ★ V8：检查指定日期 Atomic 是否已经可用
# ============================================================

def check_atomic_availability(
    raw_root: Path,
    date: str,
):
    ...
    # 不创建 en
    #
    # 防止 Enrichment 程序修改上游 Atomic。
    # --------------------------------------------------------

    if not atomic_root.exists():

        print(
            "⚠️ Atomic root does not exist."
        )
    ...
    if atomic_zh.exists() and atomic_zh.is_dir():

        zh_files = sorted(
            atomic_zh.glob("*.md")
        )

    if atomic_en.exists() and atomic_en.is_dir():
    ...
    if available:
        ...
        print(
            "✅ Atomic data is available "
            "for both ZH and EN."
        )

        reason = "available"

    elif not zh_valid and not en_valid:

        print(
            "⚠️ No valid Atomic data "
            "for either language."
        )

        reason = "no_valid_atomic"

    elif not zh_valid:

        print(
            "⚠️ ZH Atomic unavailable."
        )

        reason = "zh_atomic_unavailable"

    else:

        print(
            "⚠️ EN Atomic unavailable."
        )

        reason = "en_atomic_unavailable"
...
def wait_for_atomic(
    ...
    if required:
        ...
        print(
            "DATE ROLE: HISTORICAL"
        )

        print(
            "→ If Atomic remains unavailable "
            "after 3 checks, date will be skipped."
        )

    print(
        f"Max checks: "
        f"{ATOMIC_CHECK_RETRIES}"
    )

    print(
        f"Interval: "
        f"{ATOMIC_CHECK_INTERVAL}s"
    )
    ...
    for attempt in range(
        ...
        last_check = check_atomic_availability(
            raw_root,
            date,
        )
        ...
        if attempt < ATOMIC_CHECK_RETRIES:

            print()
            print(
                "⏳ Atomic not ready yet."
            )
    ...
    if required:
        ...
        print(
            "❌ TODAY Atomic is still unavailable "
            "after all checks."
        )
        ...
        raise RuntimeError(
            f"{date} is today, but valid Atomic "
            f"data is unavailable after "
            f"{ATOMIC_CHECK_RETRIES} checks."
        )
    ...
    #
    # 意味着：
    # 上游当天没有提供可用 Atomic 数据。
    #
    # 所以跳过并继续。
    # --------------------------------------------------------

    print(
        f"⏭️ {date} has no usable Atomic "
        "data after "
        f"{ATOMIC_CHECK_RETRIES} checks."
    )

    print(
        "⏭️ Treating this date as "
        "NO DATA."
    )

    print(
        "⏭️ Skipping date and continuing."
    )
...
def process_date(
    ...
    *,
    ...
    if required:
        ...
        print(
            "🟡 This date may be SKIPPED "
            "if upstream Atomic data is unavailable."
        )
    ...
    print()
    print(
        "STEP 1: CHECK ATOMIC AVAILABILITY"
    )

    atomic_state = wait_for_atomic(
        raw_root,
        date,
        required,
    )
    ...
    if atomic_state["skipped"]:

        print()
        print("=" * 80)

        print(
            f"⏭️ DATE SKIPPED: {date}"
        )

        print(
            "Reason: upstream Atomic "
            "data unavailable."
        )
...
def main():
    ...
    print(
        "Atomic check retries:",
        ATOMIC_CHECK_RETRIES,
    )

    print(
        "Atomic check interval:",
        f"{ATOMIC_CHECK_INTERVAL}s",
    )
    ...
    # 前天
    #   ↓
    #   Atomic 无数据 → 3次检查 → SKIP
    #   Atomic 有数据 → Enrich → Git Push
    #   ↓
    #
    # 昨天
    #   ↓
    ...
    for date in dates:
        ...
        if date == today_date:
            ...
            print(
                f"PROCESSING HISTORICAL DATE: {date}"
            )

            print(
                "🟡 HISTORICAL DATE "
                "CAN BE SKIPPED IF NO ATOMIC DATA"
            )
        ...
        if result.get(
            ...
            print(
                "→ No upstream Atomic data."
            )

            print(
                "→ No Enriched persistence."
            )
