...
def cluster_news_batch(date, items, indexes):
    ...
    prompt = f"""你正在执行748686自生长知识系统V6.5.1第一层事件聚类。
日期：{date}
...
def repair_cluster_news_batch(
    ...
    prompt = f"""修复748686 V6.5.1 ARTICLE覆盖冲突。
日期：{date}
第{attempt}次修复
...
{json.dumps(c.get('article_indexes', []))}"""
    ...
    prompt = f"""你正在执行748686自生长知识系统V6.5.1全局事件归并。
日期：{date}
轮次：{round_no}
窗口：{window_no}
...
{a['body'][:ARTICLE_AGGREGATION_CONTENT_LIMIT]}"""
    ...
    prompt = f"""你正在执行748686自生长知识系统V6.5.1第二层事件知识综合。
日期：{event['date']}
事件ID：{event['event_id']}
...
def run_one_skill(
    ...
    prompt = f"""你正在执行748686自生长知识系统V6.5.1的27 Skills深度处理。
