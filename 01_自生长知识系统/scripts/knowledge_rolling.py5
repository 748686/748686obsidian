...
重要：
    ...
    所有 Git 命令必须在 Git Repository ROOT 执行。
...
def git_sync(
    ...
    if not event_path.exists():
        ...
        raise RuntimeError(
            "❌ Git checkpoint path does not exist:\n"
            f"{event_path}"
        )
    ...
    if staged.returncode != 0:
        ...
        commit_message = (
            f"data: complete EventUnit "
            f"{date} {lang}"
        )

        print()
        print(
            f"▶️ git commit -m {commit_message}"
        )
        ...
        raise RuntimeError(
            "❌ Unable to determine staged Git changes"
        )
    ...
    print()
    print(
        "▶️ git pull --ff-only origin main"
    )
...
def process_skills_unit(
    ...
    Batch Persistence：
        ...
        Task 4 每次最多生成 30 个新的 Analysis。

        如果本批完成但整个 Skills Unit 尚未完成：
    ...
    while True:
        ...
        print(
            f"TASK 4 BATCH {batch_number} | "
            f"knowledge_task_4_skills.py | "
            f"{date}/{lang}"
        )
        ...
        if skills_complete_marker.exists():
        ...
        # Batch 未完成：
        #
        # Task 4 已经正常生成并保存本批 Analysis。
        #
        # 现在必须先 Git checkpoint，
        # 再进入下一批。
        # ----------------------------------------------------

        print()
        print(
            f"ℹ️ Task 4 batch {batch_number} "
            f"completed; _SKILLS_COMPLETE not yet present."
        )

        print(
            f"➡️ Git checkpoint required before "
            f"next Task 4 batch | {date}/{lang}"
        )
        ...
        print(
            f"➡️ Continuing Task 4 next batch | "
            f"{date}/{lang}"
        )
