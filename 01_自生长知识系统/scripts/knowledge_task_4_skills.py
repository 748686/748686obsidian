...
含义：
    ...
    每次最多生成 30 个“新的、成功验证并保存”的 Analysis。
...
之后：
    ...
    正常退出
    由外层 Workflow 执行 Git commit / push / pull
    下一轮再次调用 Task 4
...
"""
    ...
    for attempt in range(
        ...
        # ----------------------------------------------------------
        # 每次 attempt 单独保存原始 Route Router 响应。
        #
        # 如果 call_ai() 本身失败，则 raw_result 保持为空，
        # 避免误打印上一轮的响应。
        # ----------------------------------------------------------
...
- 只能返回一个 Route。
- Route 必须来自允许的 Route 列表。
- 可以直接返回 Route 名称。
- 如果返回 JSON，必须是只包含一个 Route 的 JSON 数组。
- 不要解释。
"""
            ...
            if not raw_result:
                ...
                raise ValueError(
                    "Route Router returned empty content."
                )
            ...
            route_text = raw_result

            if route_text in route_names:
            ...
            # ------------------------------------------------------
            # 3. 尝试去除 Markdown JSON code fence。
            #
            # 例如：
            #
            # ```json
            ...
            if isinstance(parsed, list):
                ...
                if len(parsed) != 1:
                    ...
                    raise ValueError(
                        "Route selection must contain "
                        "exactly one route, "
                        f"got {len(parsed)}."
                    )

                selected_route = parsed[0]
                ...
                if not isinstance(
                    ...
                    raise ValueError(
                        "Route selection item must be a string."
                    )
    ...
    raise RuntimeError(
        f"Route selection failed after "
...
"""
            ...
            # 3. 前后存在少量解释文字。
            #
            # 因此这里允许从AI响应中提取完整JSON数组。
            #
            # 注意：
            #
            #   仍然严格要求最终结果必须是JSON array。
            #   不接受JSON object。
            #   不接受单个字符串。
            #   不接受其他格式。
            # ------------------------------------------------------
            ...
            if not raw_result:
                ...
                raise ValueError(
                    "Skill Router returned empty content."
                )
...
def process_event(
    ...
    if analysis_file.exists():
        ...
        try:
            ...
            if event_id not in existing_text:

                raise ValueError(
                    f"Analysis missing Event ID: "
                    f"{event_id}"
                )

            if "## Event ID" not in existing_text:

                raise ValueError(
                    "Analysis missing required section: "
                    "## Event ID"
                )

            if "## Selected Skills" not in existing_text:
            ...
            skills_content = existing_text[
                start + len(marker):
            ]

            next_section = skills_content.find(
                "\n## "
            )
            ...
            existing_selected_skills = [
                skill
                for skill in skill_catalog
                if skill in skills_content
            ]

            if not (
                minimum
                <= len(existing_selected_skills)
                <= maximum
            ):

                raise ValueError(
                    f"Invalid existing skill count: "
                    f"{len(existing_selected_skills)} "
                    f"(expected {minimum}–{maximum})"
                )

            if not existing_selected_skills:

                raise ValueError(
                    "Existing Analysis contains no valid "
                    "catalog Skills."
                )
...
def process_unit(
    ...
    if max_new is not None:
        ...
        if max_new < 1:
            ...
            raise ValueError(
                f"--max-new must be >= 1, got {max_new}"
            )
    ...
    if max_new is not None:
        ...
        print(
            f"NEW ANALYSIS BATCH: {max_new}"
        )

    else:

        print(
            "NEW ANALYSIS BATCH: UNLIMITED"
        )
    ...
    #
    # 只有真正开始继续处理时才清除旧 marker。
    # _SKILLS_COMPLETE 本身不会被 batch 中途生成。
    # --------------------------------------------------------------
    ...
    for index, event_path in enumerate(
        ...
        if (
            ...
            print(
                f"NEW GENERATED     : {generated}"
            )
            print(
                f"PRESERVED         : {preserved}"
            )
            print(
                f"BATCH LIMIT       : {max_new}"
            )
            print(
        ...
        # 达到 batch 上限后立即返回。
        #
        # 这样最后一个新 Analysis 已经 atomic save 完成，
        # 外层 Workflow 随即可以 Git commit / push。
        # ----------------------------------------------------------
    ...
    # Final validation
    #
    # 只有所有 EventUnit 都有 Analysis 时才到这里。
    # --------------------------------------------------------------
    ...
    #
    # 绝对不会在 30 个 batch 中间写入。
    # 只有整个 Processing Unit 完整时才写。
    # --------------------------------------------------------------
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Maximum number of NEW Analysis files "
            "to generate in this invocation. "
            "Existing valid Analysis files do not count. "
            "When omitted, processing is unlimited."
        ),
    )
...
def main() -> None:
    ...
    if (
        ...
        if args.max_new is not None:
            ...
            raise ValueError(
                "--max-new requires --date and --language."
            )
