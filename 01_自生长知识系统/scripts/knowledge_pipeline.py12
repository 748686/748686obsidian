...
""" 748686 自生长知识系统 - Knowledge Pipeline V6.5.1 
       V6.5.1 changes:
       1. 保留 V6.5.0 Global Merge checkpoint / Union-Find / metadata history 架构。
       2. Stage 1A AI 聚类失败不再直接终止整个任务。
       3. AI覆盖异常自动修复；修复失败后进入 Recovery Queue。
       4. Recovery Queue 按 30 -> 15 -> 8 -> 4 -> 2 -> 1 逐级缩小。
       5. Missing-only 时，安全覆盖部分保留，Missing 文章进入 Recovery Queue。
       6. Duplicate / Extra / Malformed 时，整批隔离，避免污染后续 Global Merge。
       7. 单篇 Recovery 最终自动作为 Singleton Event Cluster 保留。
       8. 最终仍强制 ARTICLE 1..N 恰好覆盖一次，未解决文章禁止进入 Global Merge。
       9. 原始 ARTICLE 编号始终保持不变。 10. EventUnit / Skill resume 行为保持不变。 """
...
# V6.5.1：Stage 1A Recovery Queue 逐级缩小
...
def call_ai(prompt, system_prompt=None, temperature=DEFAULT_TEMPERATURE):
    ...
    req = Request(
        ...
        headers={
            ...
            "User-Agent": "748686-Knowledge-Pipeline/6.5.1"
        },
        method="POST"
    )
...
def cluster_news_batch(date, items, indexes):
    ...
    try:
        ...
        compact_prompt = f"""748686 V6.5.1 新闻事件聚类JSON修复。 日期：{date} ARTICLE范围：{json.dumps(expected)} 文章： {joined} 重新聚类。严格要求： 1. 每个ARTICLE恰好一次； 2. 同一具体现实事件合并；不同事件分开； 3. 不能确定宁可分开； 4. 每个cluster只返回cluster_id、article_indexes、event_title、event_reason； 5. event_title不超过40字；event_reason不超过80字； 6. 绝对不要输出文章正文； 7. 只输出JSON，不要代码围栏，不要解释。 格式： {{"clusters":[{{"cluster_id":"C001", "article_indexes":[1], "event_title":"事件", "event_reason":"判断"}}]}}"""
...
def repair_cluster_news_batch( date, items, indexes, broken, issues, attempt ):
    ...
    prompt = f"""修复748686 V6.5.1 ARTICLE覆盖冲突。 日期：{date} 第{attempt}次修复 真实ARTICLE：{json.dumps(expected)} 文章： {joined} 上次结果： {json.dumps(broken, ensure_ascii=False, indent=2)} 检测问题： {json.dumps(issues, ensure_ascii=False, indent=2)} 重新判断全部文章。 要求： 1. 同事件合并；不同事件分开； 2. 每篇ARTICLE恰好一次； 3. Missing=0； 4. Duplicate=0； 5. Extra=0； 6. 不得遗漏任何ARTICLE； 7. 只输出JSON，不要解释。 只输出： {{ "clusters": [ {{ "cluster_id": "C001", "article_indexes": [1], "event_title": "事件", "event_reason": "原因" }} ] }}"""
...
def _safe_covered_indexes( clusters, expected_indexes ):
    """ 只有不存在 Duplicate / Extra / Malformed 时， 才允许保留已唯一覆盖的ARTICLE。 """
    issues = inspect_cluster_assignment(
        clusters,
        expected_indexes
    )
...
def cluster_news_batch_with_repair( date, items, indexes, batch_label ):
    """ 返回： ("complete", clusters, []) ("partial", clusters, missing_indexes) ("failed", [], expected_indexes) V6.5.1原则： Missing-only 可以安全隔离Missing。 Duplicate/Extra/Malformed必须整批隔离。 AI异常不再直接终止整个任务。 """
    expected = [
...
def _make_cluster_records( batch_identifier, clusters ):
    ...
    for c in clusters:
        ...
        # Recovery使用不会和正常B001...冲突的ID空间。
        if isinstance(
            batch_identifier,
            int
        ):
            if batch_identifier >= 100000:
                final_cid = (
                    f"REC-{batch_identifier}-{cid}"
                )
            else:
                final_cid = (
                    f"B{batch_identifier:03d}-{cid}"
                )
        else:
            final_cid = (
                f"{batch_identifier}-{cid}"
            )
...
def _append_safe_clusters( allc, clusters, batch_no, expected_indexes, date, context ):
...
def _recovery_pass( date, news, indexes, recovery_pass_no, batch_size ):
    ...
    for sub_no, sub_indexes in enumerate(
        ...
        if len(sub_indexes) == 1:
            ...
            recovered.append({
                "cluster_id": (
                    f"RECOVERY-{recovery_pass_no:02d}-"
                    f"C{sub_no:03d}"
                ),
...
def build_initial_clusters(date, news):
    ...
    print("\n" + "=" * 70)
    print(
        "STAGE 1A — AI EVENT CLUSTERING V6.5.1"
    )
    print("=" * 70)
...
def merge_cluster_window( date, window, round_no, window_no ):
    ...
    for i, c in enumerate(
        ...
        blocks.append(
            f"""[CLUSTER {i}] Cluster ID： {c['cluster_id']} 原始Cluster成员： {json.dumps(c.get('member_cluster_ids', []), ensure_ascii=False)} 事件名称： {c.get('event_title', '未命名事件')} 事件判断： {c.get('event_reason', '')} 文章数量： {len(c.get('article_indexes', []))} 文章编号： {json.dumps(c.get('article_indexes', []))}"""
        )
...
def merge_all_clusters( date, clusters, news_count ):
    ...
    print("\n" + "=" * 70)
    print(
        "STAGE 1B — V6.5.1 GLOBAL EVENT MERGING"
    )
    print("=" * 70)
...
def synthesize_event(event):
    ...
    for a in event[
        ...
        blocks.append(
            f"""### 来源文章 #{a['index']} 标题：{a['title']} 来源：{a['source']} 链接：{a['source_url']} source_status：{a['source_status']} content_status：{a['content_status']} 内容： {a['body'][:ARTICLE_AGGREGATION_CONTENT_LIMIT]}"""
        )

    prompt = f"""你正在执行748686自生长知识系统V6.5.1第二层事件知识综合。 日期：{event['date']} 事件ID：{event['event_id']} 事件名称：{event['event_title']} 第一轮事件判断：{event['event_reason']} 同一事件的多来源输入： {chr(10).join(blocks)} 把来源综合成一个高质量事件知识单元。 要求： 1. 识别共同事实 2. 保留来源独有信息 3. 保留不同地区视角 4. 区分事实与推测 5. 不编造 6. source_status不是fetched时不得声称完整阅读原文 7. 冲突明确指出 8. 资料不足明确说明 输出标准中文Markdown，包含： # 事件名称 ## 事件概述 ## 核心事实 ## 多来源交叉验证 ## 不同来源独有信息 ## 不同国家 / 地区视角 ## 信息差异与冲突 ## 当前已知影响 ## 目前不能确定的事情 ## 来源 ## 事件结论"""
...
def save_event_unit( date, event, content ):
    ...
    p.write_text(
        f"""--- date: {date} event_id: {event['event_id']} type: event_unit status: completed source_count: {len(event['articles'])} timezone: Asia/Shanghai --- # {event['event_title']} > Event ID：{event['event_id']} > > 原始新闻数量：{len(event['articles'])} ## 第二层AI事件判断 {event['event_reason']} ## 第二层AI多来源综合 {content} ## 原始来源映射 {sources} """,
        encoding="utf-8"
    )
...
def mark_event_units_complete( date, n, e ):
    ...
    p.write_text(
        f"""EVENT_UNITS_COMPLETE date: {date} original_enriched_news: {n} final_event_units: {e} completed_at: {now().isoformat()} timezone: Asia/Shanghai """,
        encoding="utf-8"
    )
...
def run_one_skill( event, skill ):
    ...
    prompt = f"""你正在执行748686自生长知识系统V6.5.1的27 Skills深度处理。 事件：{event[0].get('event_title', '')} Event ID：{event[0].get('event_id', '')} Skill名称：{skill['name']} Skill规则： {skill['content']} EventUnit原文： {content[:30000]} 请严格按照该Skill完成深度处理。 不要编造；只使用EventUnit提供的信息。 输出可直接写入知识库的中文Markdown。"""
...
def run_stage_2(date):
    ...
    marker.write_text(
        f"""SKILLS_COMPLETE date: {date} events: {len(files)} skills: {len(selected)} completed_at: {now().isoformat()} timezone: Asia/Shanghai """,
        encoding="utf-8"
    )
...
def main():
    ap = argparse.ArgumentParser(
        description=
            "748686 Knowledge Pipeline V6.5.1"
    )
