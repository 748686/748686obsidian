...
存在问题：
    ...
    1. AI 可能只生成4个挖空
    2. AI 可能只生成6个挖空
    3. AI API 429 会导致完形失败
    4. AI 可能改变 ARTICLE EN 原文
    5. 重试无法保证最终一定得到10个挖空
...
V5.4：
    ...
    Python 验证
        ↓
    Unit 9 / Unit 10 使用同一篇完形文章

因此：

    Cloze Passage = 100% Python 确定性生成
    不消耗 AI API
    不受 AI 429 影响
    不存在挖空数量不足问题
    不会重新写文章
...
def build_cloze_passage_from_article(
    article_en: str,
) -> str:
    """
    根据 ARTICLE EN 由 Python 直接生成完形文章。
    ...
    article_en = _to_text(article_en)

    if not article_en:

        raise ValueError(
            "ARTICLE EN 为空，无法生成完形文章"
        )
    ...
    if len(matches) < 10:
        ...
        raise ValueError(
            "ARTICLE EN 单词数量不足10个，"
            f"实际只有{len(matches)}个"
        )
...
def generate(
    ...
    print()
    print("V5.4 最小批次独立生成模式：")
    print("  ✓ Listening A：5")
    print("  ✓ Listening B：5")
    print("  ✓ Listening C：5")
    print("  ✓ Single Choice：5 + 5")
    print("  ✓ Multiple Choice：5 + 5")
    print("  ✓ Cloze：Python Passage + 5 + 5")
    print("  ✓ Reading：5")
    ...
    print(
        "Cloze Passage："
        "Python 直接基于 ARTICLE EN 生成，"
        "不调用 AI。"
    )

    print(
        "每个 AI 生成单元独立 API → 验证 → 成功后进入下一单元。"
    )
    ...
    print("=" * 60)
    print("EXAM UNIT 8")
    print("Cloze Passage：Python 自动生成完形文章")
    print("=" * 60)

    print("✓ 不调用 AI")
    print("✓ 直接基于 ARTICLE EN")
    print("✓ 保持原文顺序")
    print("✓ Python 自动生成10个挖空")
    print("✓ 自动编号1～10")

    cloze_passage = build_cloze_passage_from_article(
        article_en
    )
    ...
    print("✓ 完形文章生成成功")
    print("✓ 已确认存在1～10号连续挖空")
    print("✓ 已确认保持 ARTICLE EN 原文")
    print("✓ 后续第1～5题、第6～10题使用同一篇文章")
    ...
    print("✓ 13个 AI 生成单元 + 1个 Python 完形单元全部完成")
    ...
    print()
    print("=" * 60)
    print("EXAM GENERATION V5.4 COMPLETE")
    print("=" * 60)
