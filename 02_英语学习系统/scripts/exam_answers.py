...
V2.4 修复：
    1. Writing 必须严格使用正式试卷中的真实 question 编号。
       不再假设 Writing 一定从第1题开始。

    2. Writing Prompt 增强 JSON 严格约束，
       降低 AI 在 analysis 中产生未转义双引号，
       导致 JSON 解析失败的问题。

    3. Writing 解析限制为简洁、具体的内容，
       避免无意义超长输出。
...
def _get_article_en(article: Any) -> str:
    ...
    for key in generic_keys:
        ...
        if isinstance(candidate, str):
            ...
            has_html_structure = bool(
                re.search(
                    r"<(?:html|body|table|tr|td|div|span|strong)\b",
                    cleaned,
                    flags=re.IGNORECASE,
                )
            )
            ...
            if (
                not has_markdown_structure
                and not has_html_structure
                and _looks_like_english_text(cleaned)
            ):
                return cleaned
...
def _build_writing_payload(
    ...
    expected_numbers_json = json.dumps(
        expected_numbers,
        ensure_ascii=False,
    )
...
你正在处理已经生成完成的正式 Writing 题目。

绝对禁止重新编号。

Writing 的 question 编号必须严格使用输入题目中的原始编号。

不得因为 Writing 只有1道题，就把题号改成1。

如果输入题目的 question 编号是2，那么：
...
{expected_numbers_json}

上面的题号是最终有效题号。

必须严格保持这些题号。

本次输入题目的实际 question 编号为：
...
answers 和 analysis 中的 question 必须完全一致。
...
严格返回一个合法 JSON object：

{{
  "answers": [
    {{
      "question": {expected_numbers[0] if expected_numbers else 1},
      "answer": "完整参考范文"
    }}
  ],
  "analysis": [
...
1. answers 数量必须等于正式 Writing 题目数量。
2. analysis 数量必须等于正式 Writing 题目数量。
3. answers 中 question 必须使用正式原题题号。
4. analysis 中 question 必须使用正式原题题号。
5. answers 和 analysis 的题号必须完全一致。
6. 不得把正式题号改成1。
7. answer 必须非空。
8. analysis 必须非空。
9. 所有字符串必须符合 JSON 语法。
...
def generate(
    ...
    print("=" * 60)
    print("STAGE 3 / 3")
    print("EXAM ANSWERS / ANALYSIS GENERATION V2.4")
    print("=" * 60)
    print()
