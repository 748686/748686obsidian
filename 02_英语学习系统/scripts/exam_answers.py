...
10. V2.4.1 兼容 Stage 2 恢复后的题目数据结构：
    ...
    if isinstance(section, dict):
        ...
        if values and all(
            ...
            for item in values:
                ...
                if (
                    "question" in item
                    or "question_text" in item
                    or "options" in item
                    or "sentence" in item
                    or "translation" in item
                ):

                    question_like.append(item)

            if len(question_like) == len(values):
...
def generate(
    ...
    single_choice_raw = exam_data.get(
        "single_choice",
        [],
    )

    multiple_choice_raw = exam_data.get(
        "multiple_choice",
        [],
    )

    cloze_raw = exam_data.get(
        "cloze",
        [],
    )

    reading_raw = exam_data.get(
        "reading",
        [],
    )
    ...
    #
    #     questions 实际为 dict，
    #     但后续代码按 list 使用 questions[0]。
    # ==============================================================

    single_choice = _normalize_question_list(
        single_choice_raw,
        "Single Choice",
    )

    multiple_choice = _normalize_question_list(
        multiple_choice_raw,
        "Multiple Choice",
    )

    cloze = _normalize_question_list(
        cloze_raw,
        "Cloze",
    )

    reading = _normalize_question_list(
        reading_raw,
        "Reading",
    )

    writing = _normalize_question_list(
        writing_raw,
        "Writing",
    )
    ...
    listening_a = _normalize_question_list(
        listening.get(
            "A",
            [],
        ),
    ...
    listening_b = _normalize_question_list(
        listening.get(
            "B",
            [],
        ),
    ...
    listening_c = _normalize_question_list(
        listening.get(
            "C",
            [],
        ),
    ...
    # Translation
    #
    # Translation 仍然保持 A / B 两个子模块。
    # --------------------------------------------------------------

    translation_a = (
        _normalize_question_list(
            translation.get(
                "A",
                [],
            ),
