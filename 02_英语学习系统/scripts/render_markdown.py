...
1. 优先读取 Agnes 的 title_zh。
2. 如果没有 title_zh，则依次兼容：
       chinese_title
       article_title_zh
       中文翻译
3. 普通正文保持默认颜色。
4. 修复目标词替换过程中空格被吞掉的问题。
5. 使用独立英文单词边界，避免：
       sleep     -> sleeping
       strong    -> stronger
       exercise  -> exercises
6. 保留原始大小写。
7. HTML 特殊字符安全转义。
8. 支持 string / list / dict / 混合结构。
9. Agnes 新数据结构优先，旧数据结构自动兼容。
10. 不增加任何 AI 调用。
11. 不改变文章正文内容。
"""
...
# 学习词文字：深蓝色
TARGET_WORD_COLOR = "#1D4ED8"

# 学习词背景：浅蓝色
TARGET_WORD_BACKGROUND = "#DBEAFE"
